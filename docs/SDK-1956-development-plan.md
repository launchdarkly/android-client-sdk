...
The resolver supports **`ConfiguredMode`** indirection: `{ configured: 'foreground' }` resolves to `input.foregroundMode`, `{ configured: 'background' }` resolves to `input.backgroundMode`. This lets the table reference the user's configured modes without hardcoding them.
...
│  Mode Resolution                     │
│                                      │
│  • Build ModeState from pending      │
│  • resolveConnectionMode(table,      │
│    modeState) → ConnectionMode       │
...
| Mapping platform state → connection mode | `resolveConnectionMode()` + `ModeResolutionTable` |
| Data source lifecycle (start, stop, rebuild on identify) | `ConnectivityManager` |
| Commanding mode switches | `ConnectivityManager` via `ModeAwareDataSource.switchMode()` |
...
Maps to JS `FDv2ConnectionMode`. Enum for now; custom modes are spec TBD (5.3.5).
...
final class ModeState {
    ...
    final ConnectionMode foregroundMode;
    final ConnectionMode backgroundMode;
}
```

Represents the current platform state and configured modes. All fields required, primitive `boolean` types. Built by ConnectivityManager from `PlatformState` + configuration.
...
final class ModeResolutionEntry {
    ...
    final Function<ModeState, ConnectionMode> resolver;  // given actual state, returns the resolved mode
}
```

### 6. `ModeResolutionTable` + `resolveConnectionMode()` (pure function)
...
final class ModeResolutionTable {
    static final ModeResolutionTable MOBILE = new ModeResolutionTable(Arrays.asList(
        ...
        new ModeResolutionEntry(
            ...
            state -> state.backgroundMode),
        new ModeResolutionEntry(
            state -> state.foreground,
            state -> state.foregroundMode)
    ));

    ConnectionMode resolve(ModeState actualState) { ... }
}
```

`resolve()` iterates entries in order. The first entry whose `matches` predicate returns `true` wins. The matched entry's `resolver` function is called with `actualState` to produce the final `ConnectionMode`. Pure function — no side effects, no platform awareness.

**Adaptation note:** This is a Java-idiomatic adaptation of Ryan Lamb's mode resolution code from js-core PR [#1146](https://github.com/launchdarkly/js-core/pull/1146) (branch `rlamb/sdk-1926/connection-mode-switching-definition`). The js-core version uses `Partial<ModeState>` for conditions (partial object matching) and TypeScript union types (`FDv2ConnectionMode | ConfiguredMode`) with string-based property lookup for mode resolution. In Java, we replace both with lambdas: conditions become `Predicate<ModeState>` (e.g., `state -> !state.networkAvailable`) and mode resolution becomes `Function<ModeState, ConnectionMode>` (e.g., `state -> state.foregroundMode`). This eliminates nullable fields, class hierarchies, and string-based indirection while preserving the same data-driven table structure and semantics.

### 7. `ModeAwareDataSource` (package-private interface)

```java
interface ModeAwareDataSource extends DataSource {
    void switchMode(ConnectionMode newMode);
}
```

FDv2DataSource implements this. ConnectivityManager checks `instanceof ModeAwareDataSource` to decide whether to use mode resolution or legacy FDv1 behavior.
...
1. **Implement `ModeAwareDataSource`** (which extends `DataSource`).
   - `ModeAwareDataSource` is a marker interface with a single method: `void switchMode(ConnectionMode newMode)`. All logic lives in `FDv2DataSource`.
...
1. **Foreground listener:** After `needsRefresh()` returns false, if the data source is a `ModeAwareDataSource`, build a `ModeState`, resolve the mode, and call `switchMode()` if the mode changed.
2. **Network listener:** Same pattern — resolve mode and call `switchMode()`.
3. **Configuration:** ConnectivityManager needs access to the `ModeResolutionTable` and configured foreground/background modes (see open question #1).
...
### Commit 1: Mode types and mode table
...
| `ModeDefinition.java` | `List<ComponentConfigurer<Initializer>>` + `List<ComponentConfigurer<Synchronizer>>` + DEFAULT_MODE_TABLE |

### Commit 2: Mode resolution types and resolver

New files only. No changes to existing code.

| File | Description |
|------|-------------|
| `ModeState.java` | Input state for mode resolution; all fields required, primitive types |
| `ModeResolutionEntry.java` | Predicate (`Predicate<ModeState>`) + resolver (`Function<ModeState, ConnectionMode>`) |
| `ModeResolutionTable.java` | Ordered list + `resolve()` method + MOBILE constant |

### Commit 3: `ModeAwareDataSource` interface + `needsRefresh()` override

| File | Description |
|------|-------------|
| `ModeAwareDataSource.java` (new) | Package-private interface extending DataSource |
| `FDv2DataSource.java` (modify) | Implement `ModeAwareDataSource`, override `needsRefresh()`, add stub `switchMode()` |

### Commit 4: `switchMode()` implementation

| File | Description |
|------|-------------|
| `FDv2DataSource.java` (modify) | Full `switchMode()` — stop synchronizers, swap to new mode's factories, resume |

### Commit 5: `FDv2DataSourceBuilder`
...
### Commit 6: ConnectivityManager mode resolution integration

| File | Description |
|------|-------------|
| `ConnectivityManager.java` (modify) | Add mode resolution for `ModeAwareDataSource` instances in foreground/network listeners |
...
### 1. How does ConnectivityManager get the mode resolution config?

ConnectivityManager needs the `ModeResolutionTable` and configured foreground/background modes. Options:

**A. New constructor parameter on ConnectivityManager:**
```java
ConnectivityManager(clientContext, dataSourceFactory, eventProcessor,
                    contextDataManager, environmentStore,
                    @Nullable ModeResolutionConfig modeConfig)
```

**B. Marker interface on the builder:**
```java
interface ModeResolutionProvider {
    ModeResolutionConfig getModeResolutionConfig();
}
```
`FDv2DataSourceBuilder` implements both `ComponentConfigurer<DataSource>` and `ModeResolutionProvider`. `LDClient` checks `instanceof` and passes config to ConnectivityManager.

**C. Put mode config into ClientContextImpl:**
Extends the existing pattern of adding internal state to ClientContext.

Recommendation: **Option B** — keeps config self-contained in the builder. `LDClient` does a simple `instanceof` check.
...
ConnectivityManager's network listener currently calls `updateDataSource(false, ...)`, which can stop the data source if the network is unavailable. For FDv2 with mode resolution, network loss → resolve to OFFLINE mode → `switchMode(OFFLINE)`. This means the network listener needs the same `instanceof ModeAwareDataSource` check as the foreground listener.

We need to ensure that when mode resolution is active, the existing `updateDataSource` logic for network changes is bypassed for `ModeAwareDataSource` instances.
