 ...
 * <p>
 * This will be used with the client's new jsonValueVariation method, and is also used internally to
 * hold feature flag values.
 * <p>
 * While the LaunchDarkly SDK uses Gson for JSON parsing, some of the Gson value types (object and
 * array) are mutable. In contexts where it is important for data to remain immutable after it is
 * created, these values are represented with {@link LDValue} instead. It is easily convertible to
 * primitive types and provides array element/object property accessors.
 *
 * @since 4.8.0
 */
...
public abstract class LDValue {
    ...
    /**
     * Returns an instance for a boolean value. The same instances for {@code true} and {@code
     * false} are always used.
     *
     ...
     * in 64-bit floating-point, which has slightly less precision than a signed 64-bit {@code
     * long}; therefore, the full range of {@code long} values cannot be accurately represented. If
     * you need to set a user attribute to a numeric value with more significant digits than will
     * fit in a {@code double}, it is best to encode it as a string.
     *
     ...
     * </code></pre>
     * If the values are all of the same type, you may also use {@link
     * LDValue.Converter#arrayFrom(Iterable)} or {@link LDValue.Converter#arrayOf(Object...)}.
     *
    ...
    /**
     * Returns an instance based on a {@link JsonElement} value. If the value is a complex type, it
     * is deep-copied; primitive types are used as is.
     *
     * @param value a nullable {@link JsonElement} reference
     * @return an LDValue containing the specified value, or {@link #ofNull()} if the value was
     * null.
     * @deprecated The Gson types may be removed from the public API at some point; it is preferable
     * to use factory methods like {@link #of(boolean)}.
     */
    @Deprecated
     ...
     * method exists only to support deprecated SDK methods where a {@link JsonElement} is needed,
     * to avoid the inefficiency of a deep-copy; application code should not use it, since it can
     * break the immutability contract of {@link LDValue}.
     *
     * @param value a nullable {@link JsonElement} reference
     * @return an LDValue containing the specified value, or {@link #ofNull()} if the value was
     * null.
     * @deprecated This method will be removed in a future version. Application code should use
     * {@link #fromJsonElement(JsonElement)} or, preferably, factory methods like {@link
     * #of(boolean)}.
     */
    @Deprecated
    public static LDValue unsafeFromJsonElement(JsonElement value) {
        return value == null || value.isJsonNull() ? ofNull() :
                LDValueJsonElement.wrapUnsafeValue(value);
    }
     ...
     *
     * @return {@code true} if this is a boolean value
     */
    ...
    /**
     * Returns this value as a boolean if it is explicitly a boolean. Otherwise returns {@code
     * false}.
     *
     * @return a boolean
     */
     ...
     * component, so {@code LDValue.of(2).isInt()} and {@code LDValue.of(2.0f).isInt()} are both
     * true.
     *
    ...
    /**
     * Returns this value as an {@code int} if it is numeric. Returns zero for all non-numeric
     * values.
     * <p>
     * If the value is a number but not an integer, it will be rounded toward zero (truncated). This
     * is consistent with Java casting behavior, and with most other LaunchDarkly SDKs.
     *
    ...
    /**
     * Returns this value as a {@code long} if it is numeric. Returns zero for all non-numeric
     * values.
     * <p>
     * If the value is a number but not an integer, it will be rounded toward zero (truncated). This
     * is consistent with Java casting behavior, and with most other LaunchDarkly SDKs.
    ...
    /**
     * Returns this value as a {@code float} if it is numeric. Returns zero for all non-numeric
     * values.
     *
    ...
    /**
     * Returns this value as a {@code double} if it is numeric. Returns zero for all non-numeric
     * values.
     *
    ...
    /**
     * Returns this value as a {@code String} if it is a string. Returns {@code null} for all
     * non-string values.
     *
    ...
    /**
     * Enumerates the values in an array or object, converting them to a specific type. Returns an
     * empty iterable for all other types.
     * <p>
    ...
    public <T> Iterable<T> valuesAs(final Converter<T> converter) {
        return new Iterable<T>() {
            ...
            public Iterator<T> iterator() {
                final Iterator<LDValue> ldValueIterator = values().iterator();
                return new Iterator<T>() {
                    @Override
                    public boolean hasNext() {
                        return ldValueIterator.hasNext();
                    }

                    @Override
                    public T next() {
                        return converter.toType(ldValueIterator.next());
                    }
                };
            }
        };
    ...
    /**
     * Returns an array element by index. Returns {@link #ofNull()} if this is not an array or if
     * the index is out of range (will never throw an exception).
     *
    ...
    /**
     * Returns an object property by name. Returns {@link #ofNull()} if this is not an object or if
     * the key is not found (will never throw an exception).
     *
    ...
    /**
     * Converts this value to a {@link JsonElement}. If the value is a complex type, it is
     * deep-copied deep-copied, so modifying the return value will not affect the {@link LDValue}.
     *
     * @return a {@link JsonElement}, or {@code null} if the value is a null
     * @deprecated The Gson types may be removed from the public API at some point; it is preferable
     * to use getters like {@link #booleanValue()} and {@link #getType()}.
     */
    @Deprecated
    ...
    /**
     * Returns the original {@link JsonElement} if the value was created from one, otherwise
     * converts the value to a {@link JsonElement}. This method exists only to support deprecated
     * SDK methods where a {@link JsonElement} is needed, to avoid the inefficiency of a deep-copy;
     * application code should not use it, since it can break the immutability contract of {@link
     * LDValue}.
     *
     * @return a {@link JsonElement}, or {@code null} if the value is a null
     * @deprecated This method will be removed in a future version. Application code should always
     * use {@link #asJsonElement()}.
     */
    @Deprecated
    ...
    // equals() and hashCode() are defined here in the base class so that we don't have to worry
    // about
    ...
    public static abstract class Converter<T> {
         ...
         * <p>
         * This method should never throw an exception; if for some reason the value is invalid, it
         * should return {@link LDValue#ofNull()}.
         *
         ...
         * Example:
         * <pre><code>
         *     List&lt;Integer&gt; listOfInts = ImmutableList.&lt;Integer&gt;builder().add(1).add(2)
         *     .add(3).build();
         *     LDValue arrayValue = LDValue.Convert.Integer.arrayFrom(listOfInts);
         * </code></pre>
         *
         * @param values a sequence of elements of the specified type
         * @return a value representing a JSON array, or {@link LDValue#ofNull()} if the parameter
         * was null
         ...
         * Example:
         * <pre><code>
         *     Map&lt;String, Integer&gt; mapOfInts = ImmutableMap.&lt;String, Integer&gt;builder()
         *     .put("a", 1).build();
         ...
         * @return a value representing a JSON object, or {@link LDValue#ofNull()} if the parameter
         * was null
     ...
     * some type, such as {@link LDValue.Converter#arrayOf(Object...)} and {@link
    ...
    public static abstract class Convert {
         ...
         */
        public static final Converter<java.lang.Boolean> Boolean =
                new Converter<java.lang.Boolean>() {
         ...
         */
        public static final Converter<java.lang.Integer> Integer =
                new Converter<java.lang.Integer>() {
         ...
         * <p>
         * Note that the LaunchDarkly service, and most of the SDKs, represent numeric values
         * internally in 64-bit floating-point, which has slightly less precision than a signed
         * 64-bit {@code long}; therefore, the full range of {@code long} values cannot be
         * accurately represented. If you need to set a user attribute to a numeric value with more
         * significant digits than will fit in a {@code double}, it is best to encode it as a
         * string.
         */
