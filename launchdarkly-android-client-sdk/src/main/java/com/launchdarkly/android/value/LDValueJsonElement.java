...
final class LDValueJsonElement extends LDValue {
    ...
    @Override
    public int intValue() {
        return type == LDValueType.NUMBER ? (int) value.getAsFloat() : 0; // don't rely on their
        // rounding behavior
    }

    @Override
    public long longValue() {
        return type == LDValueType.NUMBER ? (long) value.getAsDouble() : 0; // don't rely on
    ...
    public Iterable<String> keys() {
        if (type == LDValueType.OBJECT) {
            return value.getAsJsonObject().keySet();
        }
    ...
    public Iterable<LDValue> values() {
        switch (type) {
            case ARRAY:
                return new Iterable<LDValue>() {
                    @NonNull
                    @Override
                    public Iterator<LDValue> iterator() {
                        final Iterator<JsonElement> jsonArrayIter = value.getAsJsonArray().iterator();
                        return new Iterator<LDValue>() {
                            @Override
                            public boolean hasNext() {
                                return jsonArrayIter.hasNext();
                            }

                            @Override
                            public LDValue next() {
                                return unsafeFromJsonElement(jsonArrayIter.next());
                            }
                        };
                    }
                };
            case OBJECT:
                return new Iterable<LDValue>() {
                    ...
                    @Override
                    public Iterator<LDValue> iterator() {
                        final Iterator<Entry<String, JsonElement>> jsonObjIter = value.getAsJsonObject().entrySet().iterator();
                        return new Iterator<LDValue>() {
                            @Override
                            public boolean hasNext() {
                                return jsonObjIter.hasNext();
                            }

                            @Override
                            public LDValue next() {
                                return unsafeFromJsonElement(jsonObjIter.next().getValue());
                            }
                        };
                    }
                };
