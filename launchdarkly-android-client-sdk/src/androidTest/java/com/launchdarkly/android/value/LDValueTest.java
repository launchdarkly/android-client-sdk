...
public class LDValueTest {
    ...
    private static final LDValue anObjectValue =
            LDValue.buildObject().put("1", LDValue.of("x")).build();

    private static final LDValue aTrueBoolValueFromJsonElement =
            LDValue.fromJsonElement(new JsonPrimitive(true));
    private static final LDValue anIntValueFromJsonElement =
            LDValue.fromJsonElement(new JsonPrimitive(someInt));
    private static final LDValue aLongValueFromJsonElement =
            LDValue.fromJsonElement(new JsonPrimitive(someLong));
    private static final LDValue aFloatValueFromJsonElement =
            LDValue.fromJsonElement(new JsonPrimitive(someFloat));
    private static final LDValue aDoubleValueFromJsonElement =
            LDValue.fromJsonElement(new JsonPrimitive(someDouble));
    private static final LDValue aStringValueFromJsonElement =
            LDValue.fromJsonElement(new JsonPrimitive(someString));
    private static final LDValue anArrayValueFromJsonElement =
            LDValue.fromJsonElement(anArrayValue.asJsonElement());
    private static final LDValue anObjectValueFromJsonElement =
            LDValue.fromJsonElement(anObjectValue.asJsonElement());
    ...
    public void defaultValueJsonElementsAreReused() {
        ...
        assertSame(LDValue.of(0).asJsonElement(), LDValue.of(0).asJsonElement());
    ...
    public void objectKeysCanBeEnumerated() {
        ...
        for (String key : LDValue.buildObject().put("1", LDValue.of("x")).put("2",
                LDValue.of("y")).build().keys()) {
            keys.add(key);
        }
    ...
    public void objectValuesCanBeEnumerated() {
        ...
        for (LDValue value : LDValue.buildObject().put("1", LDValue.of("x")).put("2", LDValue.of(
                "y")).build().values()) {
    ...
    public void canUseDoubleTypeForNumberGreaterThanMaxFloat() {
        ...
        assertEquals(n, LDValue.Convert.Double.toType(LDValue.of(n)), 0);
    ...
    public void testDefaultGsonSerialization() {
        ...
        for (LDValue value : values) {
            ...
            assertEquals(value.toString(), value,
                    LDValue.normalize(gson.fromJson(value.toJsonString(), LDValue.class)));
        }
    }
    ...
    @Test
    public void testTypeAdapterDeserialization() {
        Gson gson = new Gson();
        assertEquals(LDValue.of(5), gson.fromJson("5", LDValue.class));
        assertEquals(LDValue.of(5.5), gson.fromJson("5.5", LDValue.class));
        assertEquals(LDValue.of(false), gson.fromJson("false", LDValue.class));
        assertEquals(LDValue.of(true), gson.fromJson("true", LDValue.class));
        assertEquals(LDValue.of("abc"), gson.fromJson("\"abc\"", LDValue.class));
        assertEquals(new ArrayBuilder().add(1).add(2).add(3).build(),
                gson.fromJson("[1,2,3]", LDValue.class));
        assertEquals(new ObjectBuilder().put("key", "abc").put("value", new ArrayBuilder().build()).build(),
                gson.fromJson("{\"key\": \"abc\", \"value\": []}", LDValue.class));
        assertEquals(new ObjectBuilder().put("variation", LDValue.ofNull()).build(),
                gson.fromJson("{\"variation\": null}", LDValue.class));
    }
    ...
    private <T> void testTypeConversion(LDValue.Converter<T> converter, T[] values, LDValue... ldValues) {
        ...
        ArrayList<T> lb = new ArrayList<>(Arrays.asList(values));
        assertEquals(arrayValue, converter.arrayFrom(lb));
        Iterator<T> converterIterator = arrayValue.valuesAs(converter).iterator();
        assertTrue(iterEquals(converterIterator, lb.iterator()));
        ...
        HashMap<String, T> mb = new HashMap<>();
        i = 0;
        for (T v : values) {
            mb.put(String.valueOf(++i), v);
        }
        assertEquals(objectValue, converter.objectFrom(mb));
    }

    private static <T, S> boolean iterEquals(Iterator<T> a, Iterator<S> b) {
